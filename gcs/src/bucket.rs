...
impl SnapshotMeta {
  pub fn datetime(&self) -> String {
    self.snapshot.time_created.format("%Y-%m-%d %H:%M:%S").to_string()
  }
...
pub fn search_objects_by_filename<'a>(
  ...
  objects.iter().filter_map(|object| re.captures(&object.name).map(|caps| (object, caps))).collect()
}
...
async fn get_epoch_bounds(bounds_files: &[(&GCSObject, Captures<'_>)]) -> Vec<(u64, Bounds)> {
  ...
  join_all(bounds_tasks).await.into_iter().filter_map(|r| r.ok()?).collect()
}
...
pub fn get_snapshots(objects: &[GCSObject]) -> Vec<Snapshot> {
  ...
  search_objects_by_filename(objects, &snapshot_regex).into_iter().filter_map(|(s, captures)| {
...
pub async fn get_ledger_snapshots(objects: &[GCSObject]) -> Vec<LedgerSnapshot> {
  ...
  let mut snapshots: HashMap<u64, LedgerSnapshot> = search_objects_by_filename(objects, &rocksdb_file_regex).into_iter().map(|(s, captures)| {
    let epoch: u64 = captures.get(1).unwrap().as_str().parse().unwrap();
    (
      epoch,
      LedgerSnapshot {
  ...
  let epochs_bounds = get_epoch_bounds(&search_objects_by_filename(objects, &bounds_file_regex)).await;
  ...
  let epochs_version = get_epochs_versions(&search_objects_by_filename(objects, &version_file_regex)).await;
...
fn filter_hourly_snapshots(snapshots: Vec<Snapshot>) -> Vec<Snapshot> {
  ...
  snapshots.into_iter().for_each(|snapshot| {
    grouped.entry(snapshot.time_created.date_naive()).or_default().push(snapshot);
  });
  ...
  for (_, mut group) in grouped {
    ...
    group.sort_by_key(|k| {
      ...
      let target_time = k.time_created.date_naive().and_hms_opt(target_hour, 0, 0).unwrap();
